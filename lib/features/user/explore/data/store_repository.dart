...
class StoreRepository{
  ...
  Future<List<UserModel>> featchStores()async{
    try{
      final ref  =  await _db.collection(collectionNameUsers).where('userType' , isEqualTo: "STOREOWNER").get();
      final stores = ref.docs.map((store) => UserModel.fromSnapshot(store)).toList();
      return stores;  
   ...
   Future<List<UserModel>> featchSpecialStores()async{
    try{
      final ref  =  await _db.collection(collectionNameUsers).where('userType' , isEqualTo: "STOREOWNER").limit(4).get();
      final stores = ref.docs.map((store) => UserModel.fromSnapshot(store)).toList();
      return stores;  
    }on FirebaseAuthException catch (e){
       throw TFirebaseAuthException(e.code).message;
