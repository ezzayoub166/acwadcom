...
class StoreDataCubit extends Cubit<StoreDataState> {
  ...
  // }
    ...
    try {
      ...
      if (image != null) {
        ...
        await _userRepository.updateStringFiled({"profilePicture": imageURL});
        getIt<CacheHelper>().saveValueWithKey("IMAGEURL", imageURL);
        UserModel updatedUser = await _userRepository.fetchStableData(await getIt<CacheHelper>().getValueWithKey("userID"));
  ...
  Future<void> updateUserData(String userName, String phoneNumber , String deatilsStore , String storeLink) async {
    try {
      emit(StoreDataLoading());
